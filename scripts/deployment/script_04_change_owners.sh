...
echo "RPC: $networkURL"
echo "${green}Changing owners in KarmaProxy and MechMarketplaceProxy contracts${reset}"

castSendHeader="cast send --rpc-url $networkURL$API_KEY $walletArgs"

echo "${green}Change owner in KarmaProxy${reset}"
castArgs="$karmaProxyAddress changeOwner(address) $bridgeMediatorAddress"
echo $castArgs
...
castArgs="$mechMarketplaceProxyAddress changeOwner(address) $bridgeMediatorAddress"
echo $castArgs
castCmd="$castSendHeader $castArgs"
result=$($castCmd)
echo "$result" | grep "status"
