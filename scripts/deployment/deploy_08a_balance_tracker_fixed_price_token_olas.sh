...
constructorArgs="$mechMarketplaceProxyAddress $drainerAddress $olasAddress"
...
balanceTrackerFixedPriceTokenOLASAddress=$(echo "$deploymentOutput" | grep 'Deployed to:' | awk '{print $3}')

# Get output length
outputLength=${#balanceTrackerFixedPriceTokenOLASAddress}
...
echo "$(jq '. += {"balanceTrackerFixedPriceTokenOLASAddress":"'$balanceTrackerFixedPriceTokenOLASAddress'"}' $globals)" > $globals

# Verify contract
if [ "$contractVerification" == "true" ]; then
  contractParams="$balanceTrackerFixedPriceTokenOLASAddress $contractPath --constructor-args $(cast abi-encode "constructor(address,address,address)" $constructorArgs)"
...
echo "${green}$contractName deployed at: $balanceTrackerFixedPriceTokenOLASAddress${reset}"