...
mechFactoryNvmSubscriptionTokenUSDCAddress=$(echo "$deploymentOutput" | grep 'Deployed to:' | awk '{print $3}')

# Get output length
outputLength=${#mechFactoryNvmSubscriptionTokenUSDCAddress}
...
echo "$(jq '. += {"mechFactoryNvmSubscriptionTokenUSDCAddress":"'$mechFactoryNvmSubscriptionTokenUSDCAddress'"}' $globals)" > $globals

# Verify contract
if [ "$contractVerification" == "true" ]; then
  contractParams="$mechFactoryNvmSubscriptionTokenUSDCAddress $contractPath --constructor-args $(cast abi-encode "constructor(address)" $constructorArgs)"
...
echo "${green}$contractName deployed at: $mechFactoryNvmSubscriptionTokenUSDCAddress${reset}"