...
balanceTrackerNvmSubscriptionTokenUSDCAddress=$(echo "$deploymentOutput" | grep 'Deployed to:' | awk '{print $3}')

# Get output length
outputLength=${#balanceTrackerNvmSubscriptionTokenUSDCAddress}
...
echo "$(jq '. += {"balanceTrackerNvmSubscriptionTokenUSDCAddress":"'$balanceTrackerNvmSubscriptionTokenUSDCAddress'"}' $globals)" > $globals

# Verify contract
if [ "$contractVerification" == "true" ]; then
  contractParams="$balanceTrackerNvmSubscriptionTokenUSDCAddress $contractPath --constructor-args $(cast abi-encode "constructor(address,address,address)" $constructorArgs)"
...
echo "${green}$contractName deployed at: $balanceTrackerNvmSubscriptionTokenUSDCAddress${reset}"