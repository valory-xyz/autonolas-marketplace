...
didRegistryAddress=$(jq -r '.didRegistryAddress' $globals)
transferNFTConditionAddress=$(jq -r '.transferNFTConditionAddress' $globals)
escrowPaymentConditionAddress=$(jq -r '.escrowPaymentConditionAddress' $globals)
...
echo "$(jq '. += {"subscriptionProviderAddress":"'$subscriptionProviderAddress'"}' $globals)" > $globals

# Verify contract
if [ "$contractVerification" == "true" ]; then
  contractParams="$subscriptionProviderAddress $contractPath --constructor-args $(cast abi-encode "constructor(address,address,address)" $constructorArgs)"
